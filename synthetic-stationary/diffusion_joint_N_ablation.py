"""
diffusion_joint_N_ablation.py  (synthetic-stationary)

Ablation: N-dimensional JOINT diffusion actor.
...
# ---------------------------------------------------------------------------
# Agent (same interface as DPMDAgent)
# ---------------------------------------------------------------------------

class JointDPMDAgent:
    """
    Drop-in replacement for DPMDAgent with JointDiffusionActor.
    All method signatures are identical to DPMDAgent.
    """
    ...
    def update(self, batch: Dict[str, torch.Tensor]) -> Dict[str, float]:
        ...
        # Actor update (diffusion ELBO + DPMD importance weights)
...
def train(cfg: DPMDTrainConfig | None = None, save_dir: str = "checkpoints_dpmd_joint_N"):
    ...
    print(f"Joint-N DPMD  N={cfg.N}  K={cfg.K}  T={cfg.T}  "
          f"actor_input_dim={actor_input_dim}  device={cfg.device}")
