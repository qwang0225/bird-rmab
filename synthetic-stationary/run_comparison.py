...
from env import AdaptRMABConfig, TYPE_ALPHA_MEAN, TYPE_BETA_MEAN, TYPE_NAMES
from baselines import RandomPolicy, GreedyObsPolicy, OracleLookaheadPolicy, evaluate_policy
from agent_utils import load_neurwin, load_dpmd, load_ppo, eval_agent
...
def main():
    ...
    print(f"  sigma_v={env_cfg.sigma_v}  sigma_w={env_cfg.sigma_w}  "
          f"(stationary: alpha/beta fixed at type means)")
    print("=" * 60)
    ...
    # ── Learned agents ───────────────────────────────────────────────────
    trained = [
        ("neurwin",      args.nw_ckpt,        load_neurwin),
        ("ppo",          args.ppo_ckpt,        load_ppo),
    ...
    orac_m = results.get("oracle_lookahead", results.get("oracle_greedy",
             results["random"])).mean()
    ...
    print()
    save_results_npz(Path(args.out).with_suffix(".npz"), results, "synthetic-stationary", args)
