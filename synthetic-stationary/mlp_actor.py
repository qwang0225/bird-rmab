"""
mlp_actor.py  (synthetic-stationary)

Ablation: replace diffusion actor in DPMD with a direct per-arm MLP.

Architecture:
  BeliefEncoder  : identical to DPMD (Transformer over L-step history)
  MLPActorNet    : z_i -> score_i  (per-arm MLP, one forward pass, no diffusion)
  PerArmTwinCritic: identical to DPMD

Actor loss:
  Same Q-importance-weighted objective as DPMD, but instead of the diffusion
  ELBO (noise regression), directly regress scores toward high-Q historical
  scores from the replay buffer:

      actor_loss = E_w[ ||MLP(z) - a0_old||^2 ]

  where a0_old are scores stored at collection time and
  w = exp(Q(z, topk(a0_old)) / lambda) (same as DPMD).

This is the minimal change to isolate the contribution of diffusion — only
the actor forward pass and loss differ from DPMDAgent.
...
class MLPActorAgent:
    ...
    def update(self, batch: Dict[str, torch.Tensor]) -> Dict[str, float]:
        ...
        # ── Critic update (identical to DPMD) ──────────────────────────────
        z = self._encode(obs_hist, act_hist)
        with torch.no_grad():
            z_next      = self._encode(obs_hist_next, act_hist_next)
            next_scores = self.actor(z_next)               # (B, N) — MLP, no sampling
        ...
        # ── MLP actor update ───────────────────────────────────────────────
        # Same Q-importance weights as DPMD, but regress scores directly
        # toward high-Q historical scores (no diffusion ELBO).
        z_actor  = self._encode(obs_hist, act_hist)
        a0_old   = batch["indices"].float()                # scores from replay
        with torch.no_grad():
        ...
        current_scores = self.actor(z_actor)               # (B, N)
...
def train(cfg: MLPActorConfig):
    ...
    env     = AdaptRMABEnv(env_cfg, seed=cfg.seed)
    agent   = MLPActorAgent(N=cfg.N, K=cfg.K, cfg=cfg)
    device  = agent.device

    replay = HistoryReplayBuffer(
        max_size=cfg.replay_size, n_arms=cfg.N, L=cfg.L, device=device)

    print("=" * 60)
    print(f"MLP Actor Ablation  N={cfg.N}  K={cfg.K}  T={cfg.T}  device={device}")
    print(f"epochs={cfg.epochs}  L={cfg.L}  z_dim={cfg.z_dim}")
    print("=" * 60)
    ...
    # Warmup: fill replay with random episodes
    ...
    plot_ep: List[float] = []; plot_avg10: List[float] = []
    plot_lq: List[float] = []; plot_la: List[float] = []; plot_qm: List[float] = []
    ...
    for epoch in range(start_epoch, cfg.epochs):
        ...
        plot_avg10.append(avg10); plot_lq.append(lq)
        plot_la.append(la); plot_qm.append(qm)
        ...
        with open(csv_path, "a", newline="") as f:
            csv.writer(f).writerow(
                [epoch + 1, ep_return, avg10, lq, la, qm])
...
def _plot_curve(epochs, ep_returns, avg10, loss_q, loss_actor, cfg):
    ...
    axes[0].set_title(f"MLP Actor  N={cfg.N}  K={cfg.K}")
