"""
mlp_actor.py  (synthetic-drifting)
...
Actor loss:
  Q-importance-weighted MSE toward high-Q historical scores from replay:
      actor_loss = E_w[ ||MLP(z) - a0_old||^2 ]
  where w = exp(Q(z, topk(a0_old)) / lambda)  (same as DPMD).
...
def train(cfg: MLPActorConfig):
    ...
    replay  = HistoryReplayBuffer(
        max_size=cfg.replay_size, n_arms=cfg.N, L=cfg.L, device=agent.device)

    print("=" * 60)
    print(f"MLP Actor Ablation (synthetic-drifting)  N={cfg.N}  K={cfg.K}  T={cfg.T}  device={agent.device}")
    print(f"epochs={cfg.epochs}  L={cfg.L}  z_dim={cfg.z_dim}")
    print("=" * 60)

    start_epoch = 0; best_return = -float("inf")
    if cfg.resume_path:
    ...
    with open(csv_path, "w", newline="") as f:
        csv.writer(f).writerow(["epoch", "ep_return", "avg10_return", "loss_q", "loss_actor", "q_mean"])
    ...
    returns: List[float] = []
    plot_epochs: List[int] = []; plot_ep: List[float] = []; plot_avg10: List[float] = []
    ...
    for epoch in range(start_epoch, cfg.epochs):
        ...
        if replay.size >= cfg.start_updates_after:
            for _ in range(cfg.updates_per_epoch):
                stats = agent.update(replay.sample(cfg.batch_size))
        ...
        plot_avg10.append(avg10); plot_lq.append(lq); plot_la.append(la); plot_qm.append(qm)
    ...
    axes[0].plot(plot_epochs, plot_ep, alpha=0.3, label="ep_return")
    axes[0].plot(plot_epochs, plot_avg10, linewidth=2, label="avg10")
    axes[0].set_xlabel("Epoch"); axes[0].set_ylabel("Return")
    axes[0].set_title(f"MLP Actor (synthetic-drifting)  N={cfg.N}  K={cfg.K}")
    axes[0].legend(); axes[0].grid(True, alpha=0.3)
    axes[1].plot(plot_epochs, plot_la, color="tab:orange", label="actor")
    axes[1].plot(plot_epochs, plot_lq, color="tab:blue", label="critic")
