...
class AdaptRMABConfig:
    ...
    # OU drift on per-arm (alpha, beta)
    # ou_rho=0.85, sigma_beta=0.15 -> stationary std ~ 0.29
    # Type beta_bars are [0.20, 0.80, 1.80, 3.20], gaps of 0.60/1.00/1.40
    # 2*std = 0.58 -> types 1&2 just touching, types 0&1 slightly overlapping
    # -> hard but learnable with L=40 history
    ou_rho:      float = 0.85
    sigma_alpha: float = 0.03
    sigma_beta:  float = 0.15   # stationary std~0.29; types distinguishable via history
    alpha_lo:    float = 0.65
    alpha_hi:    float = 0.99
    beta_lo:     float = 0.05
    beta_hi:     float = 6.00
...
class AdaptRMABEnv:
    ...
    def step(self, a: np.ndarray):
        ...
        # Parameter OU drift
        alpha_bar = np.array([TYPE_ALPHA_MEAN[m] for m in self.theta], dtype=np.float32)
        beta_bar  = np.array([TYPE_BETA_MEAN[m]  for m in self.theta], dtype=np.float32)
        eps_a     = self.rng.standard_normal(cfg.N).astype(np.float32)
        eps_b     = self.rng.standard_normal(cfg.N).astype(np.float32)
        self.alpha = np.clip(
            alpha_bar + cfg.ou_rho * (self.alpha - alpha_bar) + cfg.sigma_alpha * eps_a,
            cfg.alpha_lo, cfg.alpha_hi).astype(np.float32)
        self.beta  = np.clip(
            beta_bar  + cfg.ou_rho * (self.beta  - beta_bar)  + cfg.sigma_beta  * eps_b,
